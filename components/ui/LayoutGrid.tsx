...
export const LayoutGrid = ({ cards }: { cards: Card[] }) => {
	...
	return (
		<div className="w-full h-full p-10 grid grid-cols-1 md:grid-cols-4 max-w-7xl mx-auto gap-10 ">
			{cards.map((card, i) => (
				<Button key={i} borderRadius="1.75rem" duration={10000} className={cn(card.className)}>
...
const BlurImage = ({ card }: { card: Card }) => {
	...
	return <Image src={card.thumbnail} height="100" width="100" onLoad={() => setLoaded(true)} className={cn("object-cover object-top absolute inset-0 h-full w-full transition duration-200", loaded ? "blur-none" : "blur-md")} alt="thumbnail" />;
};
